...
`test` checks candidate source-file reads through path policy, reads coverage.py JSON or LCOV input files through path policy, then reports uncovered functions, classes, and interfaces. It returns `sourceReadPolicy`, `skippedSourceFiles`, and `coverageDeltaStatus` so callers can tell whether source reads were skipped and whether before/after coverage was compared. `test --write` can generate runtime tests for functions, ESM/CommonJS async JavaScript functions, simple Python dependency `Mock` calls, Java return and exception branches, and classes, while TypeScript interface-only files stay as prioritization candidates instead of producing empty runtime tests. `test --write --coverage-command <cmd>` runs the coverage command before and after writing tests through command policy, then returns `coverageRuns`, `coverageAfter`, and `coverageDelta`.

`test` 会先通过 path policy 检查候选源码文件读取，也会通过 path policy 读取 coverage.py JSON 或 LCOV 输入文件，然后输出未覆盖函数、类和接口。它会返回 `sourceReadPolicy`、`skippedSourceFiles` 和 `coverageDeltaStatus`，让调用方知道是否有源码读取被跳过，以及 before/after coverage 是否已比较。`test --write` 可以为函数、ESM/CommonJS async JavaScript 函数、简单 Python dependency `Mock` 调用、Java 返回和异常分支、类生成运行时测试；TypeScript interface-only 文件会保留为排序候选，而不会生成空的运行时测试。`test --write --coverage-command <cmd>` 会在写测试前后通过 command policy 运行 coverage 命令，并返回 `coverageRuns`、`coverageAfter` 和 `coverageDelta`。

`review` returns line-level findings, recommendations, severity summaries, actionItems, publishable `reviewComments`, and PR-comment Markdown when the diff hunk contains line metadata. It includes bug-prone addition checks such as Python mutable defaults, assignment inside JavaScript/TypeScript conditionals, and swallowed exceptions, alongside security checks for secrets, SQL/HTML/deserialization, SSRF, disabled TLS verification, weak hashes, security-sensitive randomness, dynamic execution, shell injection risks, Java `Runtime.exec` / `ProcessBuilder`, Java URL/URI SSRF, general process execution, plus performance, deployment, database, maintainability, and testing rules. Without `--diff`, default `git diff` reads are command-policy gated; `--diff` input files are read through path policy; `--github-pr` reads remote PR diffs through policy-checked `gh pr diff` or REST fallback, with public read-only GETs allowed without a token; `--write-comment` writes Markdown through Policy-as-Code; `--comment-pr` or `--publish-comment` can directly publish that generated Markdown as a policy-gated GitHub PR comment; and `github review-comments` can publish the generated file-line comments in a policy-gated batch.
...
The MCP-style server supports `initialize`, `ping`, `tools/list` with `inputSchema`, non-empty `resources/list`, `resources/templates/list`, policy-gated `resources/read`, `prompts/list`, `prompts/get`, schema-validated `tools/call`, text content, `structuredContent`, and tool-level `isError` responses. Resources expose README, Codex, Integrations, Policy docs, and `.vibeguard.yaml`; prompts expose Debug, Onboarding, Test Writer, PR Review, and GitHub PR loop workflows. The CLI `vibeguard mcp` stdio transport is covered by an end-to-end JSON-RPC smoke test that starts the process and calls `initialize`, `tools/list`, `resources/list`, `resources/read`, `prompts/list`, `prompts/get`, and `tools/call`.

MCP-style server 支持 `initialize`、`ping`、带 `inputSchema` 的 `tools/list`、非空 `resources/list`、`resources/templates/list`、经过 policy 检查的 `resources/read`、`prompts/list`、`prompts/get`、经过 schema 校验的 `tools/call`、text content、`structuredContent` 和工具级 `isError` 响应。Resources 会暴露 README、Codex、Integrations、Policy 文档和 `.vibeguard.yaml`；prompts 会暴露 Debug、Onboarding、Test Writer、PR Review 和 GitHub PR loop 工作流。CLI `vibeguard mcp` stdio transport 已有端到端 JSON-RPC smoke test 覆盖，会真实启动进程并调用 `initialize`、`tools/list`、`resources/list`、`resources/read`、`prompts/list`、`prompts/get` 和 `tools/call`。
...
After installing GitHub CLI with `winget`, an already-open PowerShell session may not see `gh` until PATH is refreshed. Reopen the terminal, run `$env:Path += ";C:\Program Files\GitHub CLI"`, or call `& "C:\Program Files\GitHub CLI\gh.exe" auth login`, then verify with `vibeguard github auth --json`.

使用 `winget` 安装 GitHub CLI 后，已经打开的 PowerShell 可能不会立刻识别 `gh`，直到 PATH 刷新。可以重新打开终端、运行 `$env:Path += ";C:\Program Files\GitHub CLI"`，或直接调用 `& "C:\Program Files\GitHub CLI\gh.exe" auth login`，然后用 `vibeguard github auth --json` 验证。
...
Spring Boot support uses the same flow and returns Spring-specific `frameworkContext` / `frameworkContexts` for dependency injection, configuration, web, and data-layer failures. Dependency-injection context includes `referencedClasses` parsed from bean names and missing types, and matching service/repository files are prioritized in `likelyFiles`.

Spring Boot 支持也使用同一流程，并针对依赖注入、配置、Web 和数据层失败返回 Spring 专用的 `frameworkContext` / `frameworkContexts`。依赖注入 context 会包含从 bean name 和 missing type 解析出的 `referencedClasses`，并在 `likelyFiles` 中优先排序匹配的 service/repository 文件。
...
```bash
vibeguard pr plan --diff reports/change.diff --write-body reports/pr-body.md --push --execute-git-plan --confirm --check-ci --wait-ci --ci-timeout 300 --github-api
...
`checks --execute` checks the generated `gh run list` command against command policy before it calls GitHub or the REST fallback. Public repository Actions runs can be read through unauthenticated REST GETs; private repositories and write operations still require `GITHUB_TOKEN` / `GH_TOKEN` or authenticated `gh`. `--wait` polls until the normalized gate reaches `pass` or `fail`, or returns `wait.status: "timeout"` with attempts and elapsed time.

`checks --execute` 会先把生成的 `gh run list` 命令交给 command policy 检查，然后才调用 GitHub 或 REST fallback。公开仓库的 Actions runs 可通过无认证 REST GET 读取；私有仓库和写操作仍需要 `GITHUB_TOKEN` / `GH_TOKEN` 或已认证的 `gh`。`--wait` 会轮询直到归一化 gate 进入 `pass` 或 `fail`，否则返回带 attempts 和 elapsed time 的 `wait.status: "timeout"`。
