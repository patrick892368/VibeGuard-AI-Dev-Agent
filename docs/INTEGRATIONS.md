...
Executed Debug/Fix Git plans route `create_pr` through the GitHub helper, so the same policy gates, `gh` execution, and token-based REST fallback are used by `fix --execute-git-plan --create-pr`.

执行 Debug/Fix Git plan 时，`create_pr` 会走 GitHub helper，因此 `fix --execute-git-plan --create-pr` 会复用同一套 policy gate、`gh` 执行和 token REST fallback。
