...
The MCP-style server supports `initialize`, `ping`, `tools/list` with `inputSchema`, empty compatible `resources/list`, `resources/templates/list`, and `prompts/list` responses, schema-validated `tools/call`, text content, `structuredContent`, and tool-level `isError` responses. The CLI `vibeguard mcp` stdio transport is covered by an end-to-end JSON-RPC smoke test that starts the process and calls `initialize`, `tools/list`, and `tools/call`.

MCP-style server 支持 `initialize`、`ping`、带 `inputSchema` 的 `tools/list`、空兼容 `resources/list`、`resources/templates/list` 和 `prompts/list` 响应、经过 schema 校验的 `tools/call`、text content、`structuredContent` 和工具级 `isError` 响应。CLI `vibeguard mcp` stdio transport 已有端到端 JSON-RPC smoke test 覆盖，会真实启动进程并调用 `initialize`、`tools/list` 和 `tools/call`。
