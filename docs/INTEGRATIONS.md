...
`checks --execute` checks the generated `gh run list` command against command policy before it calls GitHub or the REST fallback. Public repository Actions runs can be read through unauthenticated REST GETs; private repositories and write operations still require `GITHUB_TOKEN` / `GH_TOKEN` or authenticated `gh`.

`checks --execute` 会先把生成的 `gh run list` 命令交给 command policy 检查，然后才调用 GitHub 或 REST fallback。公开仓库的 Actions runs 可通过无认证 REST GET 读取；私有仓库和写操作仍需要 `GITHUB_TOKEN` / `GH_TOKEN` 或已认证的 `gh`。
