...
`pr summary` includes review findings, severity counts, and action items in the generated PR body, and returns automatic `title`, `branch`, `commitMessage`, and `automation` metadata for a follow-up GitHub PR flow. When no explicit diff is supplied through `--diff` or stdin, its default `git diff` read is command-policy gated. `--write-body` writes that body through policy so it can be reused by `github pr --body-file`.

`pr summary` 会在生成的 PR body 中包含 review findings、严重度统计和 action items，并返回自动 `title`、`branch`、`commitMessage` 和 `automation` 元数据，供后续 GitHub PR 流程使用。未通过 `--diff` 或 stdin 传入显式 diff 时，默认 `git diff` 读取会经过 command policy。`--write-body` 会经过 policy 写出正文文件，方便继续用于 `github pr --body-file`。
