...
```bash
node ./bin/vibeguard.js fix --log error.log --test "npm test" --create-branch --commit --execute-git-plan --confirm --check-ci --apply --json
```
...
```bash
node ./bin/vibeguard.js fix --log error.log --test "npm test" --create-branch --commit --push --create-pr --pr-body-file patches/pr-body.md --execute-git-plan --confirm --check-ci --apply --json
node ./bin/vibeguard.js fix --log error.log --test "npm test" --create-branch --commit --push --create-pr --pr-body-file patches/pr-body.md --execute-git-plan --confirm --check-ci --apply --github-api --json
```
...
传入 `--check-ci` 或 MCP `checkCi` 时，Fix workflow 会在 Git/PR plan 完成后通过共享 GitHub checks helper 读取生成分支的 CI 状态。Codex 应检查顶层 `ciStatus` 和 `gitExecution.ciStatus`；CI 读取不可用会作为结构化的 `ciStatus.status: "failed"` 返回，不会隐藏已经完成的 Git 结果。
...
node ./bin/vibeguard.js test --write --run --create-branch --commit --execute-git-plan --confirm --check-ci --json
node ./bin/vibeguard.js test --write --run --create-branch --commit --create-pr --execute-git-plan --confirm --check-ci --github-api --json
```
