...
The optional MCP-style server supports `initialize`, `tools/list` with JSON schemas, `tools/call`, text content, `structuredContent`, and tool-level `isError` responses. Codex should prefer `structuredContent` when available.

可选的 MCP-style server 支持 `initialize`、带 JSON schema 的 `tools/list`、`tools/call`、text content、`structuredContent` 和工具级 `isError` 响应。Codex 可用时应优先读取 `structuredContent`。
