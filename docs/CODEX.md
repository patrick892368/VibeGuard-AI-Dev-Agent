...
The optional MCP-style server supports `initialize`, `ping`, `tools/list` with JSON schemas, non-empty `resources/list`, `resources/templates/list`, policy-gated `resources/read`, `prompts/list`, `prompts/get`, schema-validated `tools/call`, text content, `structuredContent`, and tool-level `isError` responses. Resources expose README, Codex, Integrations, Policy docs, and `.vibeguard.yaml`; prompts expose Debug, Onboarding, Test Writer, PR Review, and GitHub PR loop workflows. `vibeguard mcp` is verified through a stdio JSON-RPC smoke test that covers tool, resource, prompt, and tool-call methods, and Codex should prefer `structuredContent` when available.

可选的 MCP-style server 支持 `initialize`、`ping`、带 JSON schema 的 `tools/list`、非空 `resources/list`、`resources/templates/list`、经过 policy 检查的 `resources/read`、`prompts/list`、`prompts/get`、经过 schema 校验的 `tools/call`、text content、`structuredContent` 和工具级 `isError` 响应。Resources 会暴露 README、Codex、Integrations、Policy 文档和 `.vibeguard.yaml`；prompts 会暴露 Debug、Onboarding、Test Writer、PR Review 和 GitHub PR loop 工作流。`vibeguard mcp` 已通过覆盖 tool、resource、prompt 和 tool-call 方法的 stdio JSON-RPC smoke test 验证；Codex 可用时应优先读取 `structuredContent`。
...
Debug snippets are read only when the target file is allowed by path policy; denied files may still appear as stack metadata, but their source text is not included in snippets or AI context. Debug/Fix repository metadata reads also return `metadataReadPolicy` and `skippedMetadataFiles`.
...
With `--check-ci` or MCP `checkCi`, Fix workflows read the generated branch CI status through the shared GitHub checks helper after the Git/PR plan finishes. Add `--wait-ci` or MCP `waitCi` when Codex should poll until the gate reaches `pass` or `fail`; inspect `ciStatus.wait.status`, `attempts`, and `elapsedMs` for completion or timeout. Codex should inspect top-level `ciStatus` and `gitExecution.ciStatus`; unavailable CI reads are returned as structured `ciStatus.status: "failed"` instead of hiding the completed Git result.

传入 `--check-ci` 或 MCP `checkCi` 时，Fix workflow 会在 Git/PR plan 完成后通过共享 GitHub checks helper 读取生成分支的 CI 状态。当 Codex 需要等到 CI gate 进入 `pass` 或 `fail` 时，加 `--wait-ci` 或 MCP `waitCi`；并检查 `ciStatus.wait.status`、`attempts` 和 `elapsedMs` 判断完成或超时。Codex 应检查顶层 `ciStatus` 和 `gitExecution.ciStatus`；CI 读取不可用会作为结构化的 `ciStatus.status: "failed"` 返回，不会隐藏已经完成的 Git 结果。
...
node ./bin/vibeguard.js test --write --coverage coverage.json --coverage-command "npm run coverage -- --json" --run --limit 1 --json
...
Codex should inspect `metadataReadPolicy`, `skippedMetadataFiles`, `sourceReadPolicy`, `skippedSourceFiles`, `coverage`, `coverageTargets`, `coverage.missingLines`, `uncoveredFunctions`, `uncoveredClasses`, `uncoveredInterfaces`, `classes`, `interfaces`, JavaScript `metadata.moduleSystem`, and Java `metadata.className` / `metadata.canInstantiateNoArg` / `metadata.assertionHints` before asking VibeGuard to write new tests. Repository metadata reads, candidate source-file reads, and coverage report reads are path-policy gated; skipped source files should not be read directly by Codex. TypeScript interface-only candidates are useful for prioritization but are intentionally skipped for runtime test writes unless a concrete function or class is also present. Java candidates produce JUnit 5 smoke checks, and add behavior assertions only for simple public static or safely no-arg-constructible instance methods. Inspect `coverageDeltaStatus`; if before/after reports are available, also inspect `coverageDelta.summary.averagePercentDelta`, `coverageDelta.summary.missingLinesReduced`, and file-level `status`. When using `--coverage-command` or MCP `coverageCommand`, inspect `coverageRuns` first: both before and after runs must pass before treating the generated `coverageDelta` as valid. When using `--run`, inspect `testRuns.status`, `testRuns.command`, `stdout`, `stderr`, `failureAnalysis`, and `failureAnalysis.repairPlan` before proposing a commit. When `--repair` is used, also inspect `initialTestRuns`, `repairRuns`, and the final `testRuns`; only the final `testRuns` gate Git execution. When asking for a test PR, inspect `gitPlan`, `gitPolicy`, `gitExecution`, and, with `--check-ci` / MCP `checkCi`, `ciStatus` plus `gitExecution.ciStatus`; when `--wait-ci` / MCP `waitCi` is used, also inspect `ciStatus.wait` for completed or timeout status. The generated test PR body includes a `Coverage` section when coverage status is available. Git state changes, PR creation, and CI reads still require policy confirmation where configured, and branch/commit/PR execution is blocked unless generated tests have passed. Default Test Writer branch names, commit messages, and PR titles are derived from the first generated test target, unless explicit overrides are provided. Use CLI `--github-api` or MCP `githubUseApi` when Codex should force token REST fallback for test PR creation and CI reads.

Codex 在要求 VibeGuard 写测试前，应先检查 `metadataReadPolicy`、`skippedMetadataFiles`、`sourceReadPolicy`、`skippedSourceFiles`、`coverage`、`coverageTargets`、`coverage.missingLines`、`uncoveredFunctions`、`uncoveredClasses`、`uncoveredInterfaces`、`classes`、`interfaces`、JavaScript `metadata.moduleSystem`，以及 Java `metadata.className` / `metadata.canInstantiateNoArg` / `metadata.assertionHints`。仓库元数据读取、候选源码读取和 coverage report 读取都会经过 path policy；被跳过的源码文件不应该由 Codex 直接读取。TypeScript interface-only 候选用于排序和定位，但除非同一个文件还有具体函数或类，否则会有意跳过运行时测试写入。Java 候选会生成 JUnit 5 smoke check，并且只对简单 public static 或可安全无参构造的实例方法生成行为断言。先检查 `coverageDeltaStatus`；如果有 before/after 报告，还要检查 `coverageDelta.summary.averagePercentDelta`、`coverageDelta.summary.missingLinesReduced` 和文件级 `status`。使用 `--coverage-command` 或 MCP `coverageCommand` 时，要先检查 `coverageRuns`：before 和 after 两次运行都通过后，生成的 `coverageDelta` 才能作为有效覆盖率变化。使用 `--run` 时，提交前还要检查 `testRuns.status`、`testRuns.command`、`stdout`、`stderr`、`failureAnalysis` 和 `failureAnalysis.repairPlan`。使用 `--repair` 时，还要检查 `initialTestRuns`、`repairRuns` 和最终 `testRuns`；只有最终 `testRuns` 用作 Git 执行门。生成测试 PR 时，还要检查 `gitPlan`、`gitPolicy`、`gitExecution`，以及传入 `--check-ci` / MCP `checkCi` 后的 `ciStatus` 和 `gitExecution.ciStatus`；使用 `--wait-ci` / MCP `waitCi` 时，还要检查 `ciStatus.wait` 的完成或超时状态。有 coverage 状态时，生成的测试 PR body 会包含 `Coverage` 小节。Git 状态变更、PR 创建和 CI 读取仍然会按配置需要 policy 确认，branch/commit/PR 执行会在生成测试未通过时被阻止。默认 Test Writer branch、commit message 和 PR title 会从首个生成测试目标派生，除非调用方显式覆盖。当 Codex 需要强制用 token REST fallback 创建测试 PR 和读取 CI 时，使用 CLI `--github-api` 或 MCP `githubUseApi`。
...
Generated tests may include behavior assertions for simple pure functions, ESM/CommonJS async JavaScript functions, JavaScript collection `map` / `filter` returns, `Promise.resolve(...)` returns, clear branches such as null/None checks, object-property and dictionary-field fallbacks, simple Python dependency `Mock` calls that assert both return values and call arguments, numeric lower-bound branches including `<= 0`, empty collection checks, and clear exception branches such as `throw new RangeError(...)` or `raise ValueError(...)`. Async JavaScript assertions are generated with `await` inside an async `node:test` callback, including `exports.name`, `module.exports.name`, and bracket CommonJS async exports. JavaScript module detection supports ESM, CommonJS, and CommonJS bracket exports such as `exports["name"] = value`. Python tests are generated as stdlib `unittest` cases so they can run without requiring pytest. More complex IO, database, framework dependency injection, and mock-heavy cases still require review.

生成的测试可能包含简单纯函数、ESM/CommonJS async JavaScript 函数、JavaScript 集合 `map` / `filter` 返回值、`Promise.resolve(...)` 返回值、明确分支、对象属性/字典字段 fallback、简单 Python dependency `Mock` 调用（同时断言返回值和调用参数）、常见边界值和明确异常分支的行为断言，例如 null/None 检查、包含 `<= 0` 的数值下界分支、空集合检查、`throw new RangeError(...)` 或 `raise ValueError(...)`。async JavaScript 断言会在 async `node:test` callback 中使用 `await` 生成，包括 `exports.name`、`module.exports.name` 和 bracket CommonJS async export。JavaScript 模块识别支持 ESM、CommonJS，以及 `exports["name"] = value` 这类 CommonJS bracket export。Python 测试会生成为 stdlib `unittest` 用例，因此不强制依赖 pytest。更复杂的 IO、数据库、框架依赖注入和重 mock 场景仍需要人工 review。
...
node ./bin/vibeguard.js onboard --confirm --json
```

Codex should inspect `scan.metadataReadPolicy`, `scan.skippedMetadataFiles`, `scan.dependencies`, `coreModules`, and `firstTasks` after onboarding. `scan.dependencies` lists package.json, requirements.txt, pyproject.toml, pom.xml, and Gradle dependencies with source, scope, and version when available. If dependency manifests are denied or require unconfirmed approval, they are listed in `scan.skippedMetadataFiles` and omitted from dependency inference. `coreModules` ranks likely entrypoint, routing, service, data-model, UI, and framework-config areas with bilingual reasons and representative files; `firstTasks` are repository-specific low-risk starting points with optional commands and files.

Codex 在 onboarding 后应检查 `scan.metadataReadPolicy`、`scan.skippedMetadataFiles`、`scan.dependencies`、`coreModules` 和 `firstTasks`。`scan.dependencies` 会列出 package.json、requirements.txt、pyproject.toml、pom.xml 和 Gradle 依赖，并尽量提供来源、scope 和版本。如果依赖 manifest 被拒绝或需要但尚未获得确认，它会出现在 `scan.skippedMetadataFiles`，并不会参与依赖推断。`coreModules` 会按入口、路由、服务、数据模型、UI 和框架配置等维度排序核心模块，并给出中英双语原因和代表文件；`firstTasks` 是按仓库扫描结果生成的低风险新人任务，可包含建议命令和相关文件。
...
Review findings include `file`, `line`, `severity`, `category`, `message`, and `recommendation` when line information is available. The security category covers secrets, SQL string concatenation, HTML sinks, unsafe deserialization, SSRF, disabled TLS verification, weak hashes, security-sensitive randomness, dynamic execution, shell injection risks such as `shell=True`, `os.system`, and dynamic `exec` strings, plus general process execution. The bug category currently covers Python mutable defaults, assignment inside JavaScript/TypeScript conditionals, and swallowed exceptions. Codex can use `actionItems` for planning, `reviewComments` for file-line GitHub review comments, `markdown` as a general PR comment body, `--write-comment` to write that body through Policy-as-Code, or `review --comment-pr` / `review --publish-comment` to directly publish the generated review Markdown as a policy-gated GitHub PR comment. When `review` is called without `--diff`, its default `git diff --cached` / `git diff` reads are command-policy gated. `--github-pr` and MCP `githubPr` read remote PR diffs through policy-checked `gh pr diff` or REST fallback; public read-only GETs can use the REST path without a token, while writes still require `GITHUB_TOKEN` / `GH_TOKEN`. `github review-comments` can turn the generated `reviewComments` into a batched dry-run or confirmed execution plan, and can infer the PR head SHA through policy-checked `gh pr view` or REST fallback when `--github-pr` / `githubPr` is provided and `--commit` / `commitId` is omitted. CLI `--diff` input files and MCP `review_pr.diffFile` / `summarize_pr.diffFile` / `github_review_comments.diffFile` inputs are also read through path policy.

Review findings 在有行号信息时会包含 `file`、`line`、`severity`、`category`、`message` 和 `recommendation`。security 类规则覆盖 secret、SQL 字符串拼接、HTML sink、不安全反序列化、SSRF、TLS 校验关闭、弱 hash、安全敏感随机数、动态执行、`shell=True` / `os.system` / 动态 `exec` 字符串等 shell injection 风险，以及一般进程执行。bug 类规则当前覆盖 Python mutable default、JavaScript/TypeScript 条件里的疑似赋值、以及吞掉异常。Codex 可以使用 `actionItems` 做执行计划，使用 `reviewComments` 发布文件行级 GitHub review comment，直接用 `markdown` 作为普通 PR 评论正文，用 `--write-comment` 先经过 Policy-as-Code 写出正文文件，或用 `review --comment-pr` / `review --publish-comment` 直接把生成的 review Markdown 受 policy 保护地发布为 GitHub PR comment。当 `review` 未传 `--diff` 时，默认 `git diff --cached` / `git diff` 读取会经过 command policy。`--github-pr` 和 MCP `githubPr` 会通过 policy 检查后的 `gh pr diff` 或 REST fallback 读取远端 PR diff；公开仓库只读 GET 可以无 token 走 REST，写操作仍需要 `GITHUB_TOKEN` / `GH_TOKEN`。`github review-comments` 可以把生成的 `reviewComments` 转成批量 dry-run 或确认后的执行计划；如果提供 `--github-pr` / `githubPr` 且省略 `--commit` / `commitId`，会通过 policy 检查后的 `gh pr view` 或 REST fallback 推断 PR head SHA。CLI `--diff` 输入文件和 MCP `review_pr.diffFile` / `summarize_pr.diffFile` / `github_review_comments.diffFile` 输入也会经过路径 policy 读取。

`pr summary` includes review findings, severity counts, and action items in the generated PR body, and returns automatic `title`, `branch`, `commitMessage`, and `automation` metadata for a follow-up GitHub PR flow. `pr plan` uses the same metadata to return `gitPlan`, `gitPolicy`, and optional `gitExecution` for branch, stage, commit, push, and PR creation. Git plan policy checks every staged file in `git add` through path policy, and also checks PR body files before a generated `create_pr` command can run. With `--check-ci` or MCP `checkCi`, `pr plan` also returns `ciStatus`; before Git execution it is a dry-run `gh run list` command, and after successful Git/PR execution it reads workflow runs for the generated head branch through the same policy-gated GitHub checks helper. `--wait-ci` or MCP `waitCi` adds CI polling and `ciStatus.wait` metadata. When no explicit diff is supplied through `--diff` or stdin, the default `git diff` read is command-policy gated. `--write-body` writes the PR body through policy so it can be reused by `github pr --body-file` or the generated `create_pr` plan.

`pr summary` 会在生成的 PR body 中包含 review findings、严重度统计和 action items，并返回自动 `title`、`branch`、`commitMessage` 和 `automation` 元数据，供后续 GitHub PR 流程使用。`pr plan` 会使用同一份元数据返回 `gitPlan`、`gitPolicy` 和可选 `gitExecution`，覆盖 branch、stage、commit、push 和 PR 创建。Git plan policy 会对 `git add` 中每个 staged 文件做 path policy 检查，也会在生成的 `create_pr` 命令运行前检查 PR body 文件。传入 `--check-ci` 或 MCP `checkCi` 时，`pr plan` 还会返回 `ciStatus`；Git 执行前它是 dry-run 的 `gh run list` 命令，Git/PR 执行成功后会通过同一条受 policy 保护的 GitHub checks helper 读取生成 head branch 的 workflow runs。`--wait-ci` 或 MCP `waitCi` 会增加 CI 轮询和 `ciStatus.wait` metadata。未通过 `--diff` 或 stdin 传入显式 diff 时，默认 `git diff` 读取会经过 command policy。`--write-body` 会经过 policy 写出 PR body，方便继续用于 `github pr --body-file` 或生成的 `create_pr` plan。

Through MCP-style calls, use `summarize_pr` for PR body and metadata only, or `plan_pr` when Codex needs the protected Git/PR command plan in the same response.

通过 MCP-style 调用时，只需要 PR body 和元数据就用 `summarize_pr`；需要同一响应里带受保护的 Git/PR 命令计划时用 `plan_pr`。
...
node ./bin/vibeguard.js github checks --branch codex/fix-bug --limit 5 --execute --wait --wait-timeout 300 --json
```

`github checks --execute` first builds the `gh run list` dry-run command and checks it against command policy before reading remote CI state. `--wait` polls until the normalized gate reaches `pass` or `fail`, or returns timeout metadata for Codex to surface.

`github checks --execute` 会先生成 `gh run list` dry-run 命令，并在读取远端 CI 状态前经过 command policy 检查。`--wait` 会轮询直到归一化 gate 进入 `pass` 或 `fail`，否则返回供 Codex 暴露的 timeout metadata。
...
When `gh` is unavailable, execute mode can use the GitHub REST API fallback; public read-only GETs such as PR diff, PR head, and Actions runs can be unauthenticated, while PR creation and PR comments still require `GITHUB_TOKEN` or `GH_TOKEN`. `--github-api` / `useApi` can also force that path even if `gh` exists. Policy confirmation is still required for PR creation and PR comments.

当本机没有 `gh` 时，execute 模式可以使用 GitHub REST API fallback；PR diff、PR head、Actions runs 等公开只读 GET 可以无认证读取，但创建 PR 和发布 PR comment 仍需要 `GITHUB_TOKEN` 或 `GH_TOKEN`。即使存在 `gh`，也可以用 `--github-api` / `useApi` 强制走这条路径。创建 PR 和发布 PR comment 仍然需要 policy 确认。
...
- Treat `gitPlan` as reviewable until `--execute-git-plan --confirm --apply` is present; inspect `gitPolicy.results` and `gitPolicy.pathResults`, including staged-file path checks, before execution. / 没有 `--execute-git-plan --confirm --apply` 时，`gitPlan` 只是可审查计划；执行前要检查 `gitPolicy.results` 和 `gitPolicy.pathResults`，包括 staged 文件路径检查。
