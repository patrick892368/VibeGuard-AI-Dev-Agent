...
The optional MCP-style server supports `initialize`, `ping`, `tools/list` with JSON schemas, empty compatible `resources/list`, `resources/templates/list`, and `prompts/list` responses, schema-validated `tools/call`, text content, `structuredContent`, and tool-level `isError` responses. `vibeguard mcp` is verified through a stdio JSON-RPC smoke test, and Codex should prefer `structuredContent` when available.

可选的 MCP-style server 支持 `initialize`、`ping`、带 JSON schema 的 `tools/list`、空兼容 `resources/list`、`resources/templates/list` 和 `prompts/list` 响应、经过 schema 校验的 `tools/call`、text content、`structuredContent` 和工具级 `isError` 响应。`vibeguard mcp` 已通过 stdio JSON-RPC smoke test 验证；Codex 可用时应优先读取 `structuredContent`。
