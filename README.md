...
- `vibeguard pr summary`: 从 diff 生成包含 review findings、actionItems、自动 PR title、branch name 和 commit message 的 GitHub-ready PR body；默认读取 `git diff` 前会先过 command policy，也可用 `--github-pr` 读取远端 PR diff；可用 `--write-body` 经过 policy 写出 PR body 文件。Builds a GitHub-ready PR body plus automatic PR title, branch name, and commit message with review findings and actionItems from a diff; default `git diff` reads pass command policy first, `--github-pr` can read remote PR diffs, and `--write-body` can write the PR body file through policy.
