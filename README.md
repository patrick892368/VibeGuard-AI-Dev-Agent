...
- `vibeguard mcp`: 启动 MCP-style stdio server，支持 `initialize`、`ping`、`tools/list` schema、空 `resources/list` / `resources/templates/list` / `prompts/list` 兼容响应、structured tool output、受 read policy 保护的 log/patch/diff/coverage 文件输入、`debug_error` AI patch artifact、`apply_patch_safely`、`plan_pr`，以及 `github_auth`、GitHub PR dry-run、普通/批量 review comments、checks 等 Codex 工作流工具。Starts an MCP-style stdio server with `initialize`, `ping`, `tools/list` schemas, empty compatible `resources/list` / `resources/templates/list` / `prompts/list` responses, structured tool output, read-policy-protected log/patch/diff/coverage file inputs, `debug_error` AI patch artifacts, `apply_patch_safely`, `plan_pr`, and Codex workflow tools such as `github_auth`, GitHub PR dry-runs, general/batched review comments, and checks.
