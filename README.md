...
- `vibeguard onboard`: 扫描仓库并生成中英双语 onboarding / architecture 文档、结构化 `coreModules` 核心模块、仓库相关 Mermaid 架构图、结构化 firstTasks 新人任务建议和 `commandChecks` 命令可用性说明。Scans a repository and can generate bilingual onboarding / architecture docs, structured `coreModules`, repository-specific Mermaid architecture diagrams, structured firstTasks for newcomer work, and `commandChecks` for command readiness notes.
