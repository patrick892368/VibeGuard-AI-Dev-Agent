...
- `vibeguard test`: 扫描测试候选，并可通过 read policy 读取 coverage.py JSON / LCOV，排序未覆盖文件、函数、类和接口，也可比较 before/after coverage。Scans source files for test candidates, reads coverage.py JSON / LCOV through read policy, can prioritize uncovered files, functions, classes, and interfaces, and can compare before/after coverage.
...
GitHub detect 和 REST fallback 需要的 `git remote get-url origin`、`git branch --show-current` 等 prerequisite 命令，也会在执行前经过 command policy。
...
- PR 创建调度、普通 PR comment dry-run、单条/批量 review comment dry-run、GitHub detect/PR prerequisite policy、CI checks execute policy、policy confirmation 和 REST fallback。PR creation dispatch, general PR comment dry-runs, single/batched review comment dry-runs, GitHub detect/PR prerequisite policy, CI checks execute policy, policy confirmation, and REST fallback.
...
- coverage.py JSON / LCOV 输入文件的 path policy 读取、未覆盖文件排序、missing line 到函数/类/接口映射、before/after coverage delta，以及生成测试后的 policy-gated test run。Path-policy reads for coverage.py JSON / LCOV input files, uncovered file prioritization, missing-line-to-function/class/interface mapping, before/after coverage deltas, and policy-gated test runs after generating tests.
