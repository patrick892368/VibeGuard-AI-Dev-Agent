...
- `vibeguard debug`: 通过 read policy 读取报错日志，解析 Python、Django、Node.js、Java/Spring Boot 报错；Django `TemplateDoesNotExist` 会同时定位 URL、view、template 候选和 settings；Spring Boot 依赖注入错误会解析 bean name / missing type 并优先相关 Service / Repository；Java 栈会优先用完整包名消歧同名源码文件，定位可能文件，并输出结构化 `explanation` 解释为什么失败。Reads error logs through read policy and parses Python, Django, Node.js, and Java/Spring Boot errors; Django `TemplateDoesNotExist` locates URL, view, template candidates, and settings together; Spring Boot dependency-injection errors parse bean names / missing types and prioritize related services and repositories; Java stack traces use fully qualified package names to disambiguate duplicate source filenames, find likely files, and return a structured `explanation` for why the failure happened.
