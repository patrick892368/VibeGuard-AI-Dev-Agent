...
- `vibeguard test --write`: 经过 policy 后写入基础测试，支持 ESM/CommonJS Node 模块，会为简单纯函数生成行为断言，可用 `--run` 继续通过 command policy 执行生成的测试，并分类失败日志。Writes basic tests after policy checks, supports ESM/CommonJS Node modules, generates behavior assertions for simple pure functions, can use `--run` to execute generated tests through command policy, and classifies failed run logs.
