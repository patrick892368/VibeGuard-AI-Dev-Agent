...
function summarizeFixture(fixture, tempRoot, result, apply, run = 1) {
  ...
  return {
    ...
    patchRecoveryStatus: result.recovery?.status || null,
    patchRecoveryStrategy: result.recovery?.patchSource?.strategy || result.recovery?.strategy || null,
    patchDiagnostics: result.patchDiagnostics || null,
...
function buildHistoryEntry(report) {
  return {
    ...
    results: report.results.map((result) => ({
      ...
      patchRecoveryStatus: result.patchRecoveryStatus,
      patchRecoveryStrategy: result.patchRecoveryStrategy,
